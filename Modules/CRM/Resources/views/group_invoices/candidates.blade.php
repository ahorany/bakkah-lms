            ...
            <table class="table table-responsive table-striped table-hover table-total-info" style="border: 1px solid #dddddd;">
                ...
                @foreach ($cartMaster->carts as $cart)
                    <tr data-tr="{{$cart->id}}" {{(($cart->userId->id/2)==0) ? 'class=active' : '' }}>
                        <td style="padding: 0 10px;text-align: center;"><button data-id="{{$cart->id}}" class="btn btn-sm btn-danger btn-table btn-delete" style="visibility:visible;padding: 0;">
                        ...
                        {{-- <td data-id="{{$cart->id}}"><a class="btn btn-sm btn-danger btn-table btn-delete" href="{{route('crm::rfq.register.delete', ['id'=>$cart->id])}}"><i class="fa fa-trash"></i> {{__('education.delete')}}</a></td> --}}
                    </tr>
                @endforeach
