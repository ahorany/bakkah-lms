...
<form id="post-search" class="form-inline" method="get" action="{{route('crm::group_invoices.index')}}">
        {!!Builder::Hidden('post_type', $post_type)!!}
        <?php
            $type_id = $cartMaster->type_id??request()->type_id;
            if(is_null($type_id)){
...
<script>
    jQuery(function (){
        ...
        jQuery('[name="clear"]').click(function (){
            ...
            jQuery('[name="invoice_number"], [name="follow_up_date_from"], [name="follow_up_date_to"]').val('');
            return false;
        });
    });
</script>
