...
@section('content')
    <div class="card p-5 user-info">
        <div class="d-flex" style="justify-content: space-between;">
            ...
            <a style="width: 85px;" href="{{CustomRoute('user.exam',$exam_id)}}" class="main-color back_button">
                                ...
                                <td>
                                    ...
                                    <div class="progress">
                                        <div class="mx-auto progress-bar " role="progressbar" style="width: {{($data->result != null) || ($data->total != null) ? number_format((($data->result / $data->total) * 100), 2, '.', ',') . '%' : 0}};" aria-valuenow="{{$data->result}}" aria-valuemin="0" aria-valuemax="100">
                                            {{($data->result != null) || ($data->total != null) ? number_format((($data->result / $data->total) * 100), 0, '.', ','). '%' : 0}}
                                        </div>
                                    </div>
                                </td>
                                <td>{{$data->total??0}}</td>
                    ...
                    @foreach($unit_marks as $data)
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="progress">
                                    <div class="mx-auto progress-bar @if($progress < 50) bg-danger @endif"  role="progressbar" style="width: {{$progress}}%;" aria-valuenow="{{$progress}}" aria-valuemin="0" aria-valuemax="100">{{($progress > 0) ? number_format($progress, 0, '.', ',') . '%' : '0' }}</div>
                                </div>
                            </td>
                            <td>
                                {{$data->total_marks}}
