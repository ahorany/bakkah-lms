            ...
            <span style="color: #999; font-size: 13px;display: block;">
                ...
                @foreach($cart->cartFeatures as $cartFeature)
                    <span class="text-success d-block" style="font-size: 14px;">
                        {{App\Helpers\Lang::TransTitle($cartFeature->trainingOptionFeature->feature->title)??null}}
                        @if ($cartFeature->trainingOptionFeature->feature->id == 5)
                            <small class="text-danger"> ({{App\Helpers\Lang::TransTitle($cartFeature->trainingOptionFeature->excerpt)??null}})</small>
                        @endif
                        : <small>{{NumberFormatWithComma($cartFeature->price)??null}} {{$cartMaster->coin->en_name??null}}</small></span>
                @endforeach
            </span>
            <?php
            $class = [
