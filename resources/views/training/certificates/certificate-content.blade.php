...
<body>
  <div style="position: relative;width:3508px;height:2480px;width: 800px;height:562px;">
    ...
    <div style="width: 75%;height: 562px;background-color: #ffffff;float: left;background: url(https://wp.bakkah.net.sa/wp-content/themes/bakkah-new/includes/generate_ji_emails/vendor/phpoffice/phpword/gen_files/certificate/bg-logo.png) no-repeat;background-size: 130px;background-position: right top;border-bottom: 5px solid var(--mainColor);position: relative;">
      ...
      <h3 style="font-weight: bold;font-size: 30px;">{{$post->userId->trans_name??null}}</h3>
      ...
      <h5 style="font-size: 22px;margin: 0;padding-top: 10px;">{{$post->trainingOption->training_name??null}}</h5>
      <p style="margin:0;margin-bottom: 15px;margin-top: 15px;font-size: 20px;font-weight: 300;">{{$post->session->published_from??null}} - {{$post->session->published_to??null}}</p>
      ...
      <div class="row" style="display: inline-block;width: 100%;margin-bottom: 5px;margin-top: 20px;">
        <div style="float: left;padding-left: 45px;">
          <img src="https://wp.bakkah.net.sa/wp-content/uploads/2015/03/aPHRi_certificate.png" style="width: 170px;display: block;margin: 0 auto;margin-top: 10px"></div>
      ...
      <div class="row" style="padding-bottom: 5px;position: absolute;bottom: 0px;display: inline-block;transform: translateX(-100%);width: 100%;">
        <p style="margin: 0;font-size: 6px;padding: 0 40px;">{{$post->course->partner->trans_excerpt}}</p>
      </div>
      </div>
    </div>
  </div>
