...
<div class="attendance-content" style="margin: 0 auto;">
    <div style="padding: 0 90px;">
        ...
        <h6 style="background-color: var(--mainColor);color: #fff;margin: 0;margin-top: 20px;padding: 5px 10px;width: 160px;text-align: center;">{{date_format(now(), 'l, F d, Y')}}</h6>
        ...
        <p style="margin-top: 70px;font-size: 18px;font-weight: 300;position: relative;text-align: justify;color: #4e4e4e;">This is to confirm that <b><span style="font-weight: 600;">{{$cart->userId->trans_name??null}}</span></b> has successfully completed
            ...
            @endif
                <b><span style="font-weight: 600;">{{$cart->course->ar_disclaimer??$cart->course->en_title}} {{($training_option !=353 && $training_option !=383)?$cart->trainingOption->constant->trans_name : null}}</span></b> with {{__('education.app_title')}}.
