...
@section('useHead')
    @include('SEO.head', ['eloquent'=>$post??null, 'type' => 'webinar'])
@endsection
                ...
                <div class="col-md-12">
                                ...
                                <div class="event-img">
                                    ...
                                    @if ($session_start_time >= $today_now)
                                        <div class="time-countdown">
                                            ...
                                            <div><span id="days"></span>{{__('education.Days')}}</div>
                                            <div><span id="hours"></span>{{__('education.Hours')}}</div>
                                            <div><span id="minutes"></span>{{__('education.Minutes')}}</div>
                                            <div><span id="seconds"></span>{{__('education.Seconds')}}</div>
                    ...
                    <div class="event-info border p-3 rounded my-5">
                        <div class="row align-items-center">
                            <div class="col-md-4 mb-4 mb-md-0">
                                <i class="far fa-clock main-color"></i> <strong>{{__('education.start_time')}}</strong> <span>{{ $post->session_start }} | {{ $session_time_from }}</span>
                            </div>
                            <div class="col-md-4 mb-4 mb-md-0">
                                <i class="fas fa-flag main-color"></i> <strong>{{__('education.end_time')}}</strong> <span>{{ $post->session_end }} | {{ $session_time_to }}</span>
                            </div>

                            <div class="col-md-4">
                                @if ($session_start_time >= $today_now)
                                        <button id="registerBtn" class="btn btn-primary btn-block btn-lg">{{__('education.Register')}}</button>
                                @else
                                    <button class="btn btn-primary btn-block btn-lg" disabled>{{__('education.registration_over')}}</button>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="event-tabs my-5">
                        <div class="nav nav-pills flex-column flex-sm-row" id="pills-tab" role="tablist">
                            <span class="flex-sm-fill nav-link btn btn-light active" data-toggle="pill">{{__('education.Webinar Description')}}</span>
                        ...
                        <div class="tab-content border rounded p-3" id="pills-tabContent">
                            <div class="tab-pane fade show active" id="event-description" role="tabpanel" aria-labelledby="pills-home-tab">
                                ...
                                {{-- @if (!is_null($post->zoom_link) && ($session_end_time >= $today_now))
                                    <a href="{!! $post->zoom_link !!}" class="btn btn-primary" target="_blank">{{__('education.Join us Now!')}}</a>
                                @endif --}}
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title boldfont" id="webinar_registrationTitle">{{__("education.Webinar Registration")}}</h5>
            ...
            <form method="post" id="registration_form" action="{{route('education.static.webinarRegistration', ['slug'=>$post->slug])}}" target="hidenFrame">
                <div class="container">
                  <div class="row">
                    @csrf
                    {!! Recaptcha::execute() !!}
                    ...
                    <div class="col-12 col-md-6">
                      ...
                      <div class="form-group">
                        ...
                        <button name="registerBtn" type="submit" class="btn btn-primary">{{__("education.Register")}}</button>
                          ...
                          <div class="row align-items-center">
                            ...
                            <div class="col-md-12 mb-4 mb-md-4" name="the_excerpt">
                              ...
                              <div class="col-md-12 mb-2 mb-md-0">
                                  <i class="far fa-clock main-color"></i> <strong>{{__("education.start_time")}}</strong> <span>{{ $session_time_from }}</span>
                              </div>
                              <div class="col-md-12 mb-2 mb-md-0">
                                  <i class="fas fa-flag main-color"></i> <strong>{{__("education.end_time")}}</strong> <span>{{ $session_time_to }}</span>
                              </div>
                          </div>
                      </div>
