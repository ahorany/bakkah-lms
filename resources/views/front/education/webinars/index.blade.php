...
@section('content')
    ...
    @include(FRONT.'.education.Html.page-header', ['title'=>__('education.webinars')])
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="btn-group event-cat" role="group">
                        <a href="{{route('education.static.webinars')}}" class="btn @if(is_null(request()->event)) active @endif"> <i class="fas fa-globe-asia"></i> <span>{{__('education.all_events')}}</span></a>
                        <a href="{{route('education.static.webinars')}}?event=happening" class="btn @if(!is_null(request()->event) && request()->event == 'happening') active @endif"> <i class="fas fa-bolt"></i> <span>{{__('education.Happening')}}</span></a>
                        <a href="{{route('education.static.webinars')}}?event=upcoming" class="btn @if(!is_null(request()->event) && request()->event == 'upcoming') active @endif"> <i class="fas fa-star"></i> <span>{{__('education.Upcoming')}}</span></a>
                    </div>
                </div>
                    ...
                    <div class="event-box box-shadow">
                        <div class="img-wrapper">
                            ...
                            @if ($session_start_time < $today_now)
                                <small class="closed">{{__('education.Closed')}}</small>
                            @else
                                <small class="opened">{{__('education.Open')}}</small>
                            @endif

                            <a href="{{route('education.static.webinars.single', ['slug'=>$post->slug])}}">
                        ...
                        <div class="p-3 text-center">
                            <h2 class="boldfont"><a href="{{route('education.static.webinars.single', ['slug'=>$post->slug])}}"> {{$post->trans_title}} </a></h2>
                            ...
                            <a href="{{route('education.static.webinars.single', ['slug'=>$post->slug])}}" class="btn btn-block btn-secondary">{{__('education.Read More')}}</a>
                        </div>
                    </div>
                </div>
                @endforeach
