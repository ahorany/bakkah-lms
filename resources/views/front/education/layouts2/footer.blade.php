...
<script>/* type="application/javascript" */
    ...
    methods: {
        ...
        changeCartMaster:function(data){
            alert('fdfd')
            this.cartMaster =data;
        },
        ...
        AddCartFeature: function(cart_id, feature_id, price, course_price, event) {
            ...
            .then(function (response){
                ...
                self.changeCartMaster(response.data.CartMaster);
        ...
        PromoCodeCart: function(cart_id, event) {
            ...
            .then(response => {
                ...
                // vm_this.$set(vm_this.features.total, cart_id, response.data.cart.total);
                vm_this.promo_code = response.data.cart.promo_code;


                /*vm_this.carts = vm_this.carts.filter(function(elem){
                    if(elem.id == response.data.cart.id) {
                        console.log('test');
                        return elem = response.data.cart;
                        // return elem = response.data.cart;
                    }
                    return elem;
                });*/

                // vm_this.$set(vm_this.carts[response.data.cart.id], response.data.cart, response.data.cart);
                // console.log(vm_this.carts);
                // console.log(vm_this.promo_code);

                vm_this.carts.forEach(function(elem,index){
                    if(elem.id == response.data.cart.id) {
                        // vm_this.carts[index] = response.data.cart;
                        vm_this.$set(vm_this.carts, index, response.data.cart);
                    }
                });
