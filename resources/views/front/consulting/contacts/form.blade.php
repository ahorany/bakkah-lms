...
<div class="row">
    <div class="col-md-12">
        <form action="{{route('consulting.static.contactusStore')}}" method="post" >
            @csrf
            {!! Recaptcha::execute() !!}
                    ...
                    <div class="form-group">
                        <select name="request_type" class="form-control @error('request_type') is-invalid @enderror" >
                            ...
                            @foreach($posts as $request)
                                <?php
                                $selected = '';
                                if(old('request_type')==$request->basic_id){
                                    $selected = 'selected="selected"';
                                }else if($request_type==$request->basic_id){
                                    $selected = 'selected="selected"';
                                }
                                ?>
                                <option value="{{$request->basic_id}}" {{$selected}}>{{$request->title}}</option>
                            @endforeach
                        </select>
                        @error('request_type')
                            <small class="text-danger">{{$message}}</small>
